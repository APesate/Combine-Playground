...
# Operators
An `Operator` is an object that acts both like a subscriber and a publisher. They are classes that adopt both the Subscriber protocol and Publisher protocol. They support subscribing to a publisher, and sending results to any subscribers.

You can create chains of these together, for processing, reacting, and transforming the data provided by a publisher, and requested by the subscriber, both the Output and Failure type.
...
let combineSubject1 = PassthroughSubject<Bool, Never>()
let combineSubject2 = PassthroughSubject<Bool, Never>()
...
print(note: "Sending `true` on `combineSubject1`")
combineSubject1.send(true)
...
catchSubject
	...
	.catch { (error) -> AnyPublisher<Int, OperatorError> in
		print(note: "An error was catched: \(error)")

...
simplePublisher
	.sink(receiveCompletion: { (complitionEvent) in
		print(event: complitionEvent)

		switch complitionEvent {
...
Subscribers can support cancellation, which terminates a subscription and shuts down all the stream processing prior to any Completion sent by the publisher. Both Assign and Sink conform to the `Cancellable protocol`. Calling `cancel() `frees up any allocated resources. It also stops side effects such as timers, network access, or disk I/O.
